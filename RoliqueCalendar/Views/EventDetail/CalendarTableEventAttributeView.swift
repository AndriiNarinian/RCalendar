...
//    }
    ...
    var tableViewHeight: CGFloat {
        return kHeaderHeight + (kRowHeight * CGFloat(self.event?.calendars.count ?? 0))
    }
...
//    func getHeightForTableDataKey() -> CGFloat {
...
//    }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return event?.calendars.count ?? 0
    }

    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = UITableViewCell(style: .default, reuseIdentifier: "CalendarTableEventAttributeView")
        guard let calendarData = event?.calendars[safe: indexPath.row] else { return UITableViewCell() }
